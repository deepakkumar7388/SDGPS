...
interface GatePassDao {
    ...
    @Query("SELECT * FROM gate_passes WHERE json_extract(passData, '$.destinationCampus') IS NULL AND SUBSTR(json_extract(passData, '$.applyDate'), 1, 10) >= SUBSTR(:todayStart, 1, 10) AND SUBSTR(json_extract(passData, '$.applyDate'), 1, 10) <= SUBSTR(:todayEnd, 1, 10) AND json_extract(passData, '$.status') IN ('pending', 'approving', 'approved') AND (json_extract(passData, '$.applyEmail') != :loginUserEmail OR :userRole IN ('principal', 'hod')) ORDER BY json_extract(passData, '$.applyDate') DESC")
    ...
    @Query("SELECT * FROM gate_passes WHERE json_extract(passData, '$.destinationCampus') IS NULL AND SUBSTR(json_extract(passData, '$.applyDate'), 1, 10) >= SUBSTR(:todayStart, 1, 10) AND SUBSTR(json_extract(passData, '$.applyDate'), 1, 10) <= SUBSTR(:todayEnd, 1, 10) AND json_extract(passData, '$.status') IN ('approved') ORDER BY json_extract(passData, '$.applyDate') DESC")
    fun getActiveGatePassesBySecurity(todayStart: String, todayEnd: String): List<GatePassEntity>

    @Query("SELECT * FROM gate_passes WHERE json_extract(passData, '$.destinationCampus') IS NULL AND (SUBSTR(json_extract(passData, '$.applyDate'), 1, 10) < SUBSTR(:todayStart, 1, 10) OR json_extract(passData, '$.status') NOT IN ('pending', 'approving', 'approved')) ORDER BY json_extract(passData, '$.applyDate') DESC")
    fun getHistoricalGatePasses(todayStart: String): List<GatePassEntity>

    @Query("SELECT * FROM gate_passes WHERE json_extract(passData, '$.destinationCampus') IS NULL AND SUBSTR(json_extract(passData, '$.applyDate'), 1, 10) >= SUBSTR(:startDate, 1, 10) AND SUBSTR(json_extract(passData, '$.applyDate'), 1, 10) <= SUBSTR(:endDate, 1, 10) ORDER BY json_extract(passData, '$.applyDate') DESC")
